...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    mean_ap = evaluate_detections(all_boxes, output_dir, dataset)
    return mean_ap


def evaluate_detections(box_list, output_dir, dataset):
    write_voc_results_file(box_list, dataset)
    mean_ap = do_python_eval(output_dir)
    return mean_ap
...
if __name__ == '__main__':
    ...
    net = build_ssd_gmm('test', 300, num_classes)  # initialize SSD
    net = nn.DataParallel(net)
    net.load_state_dict(torch.load(args.trained_model))
    net.eval()
    print('Finished loading model!')
    # load data
    dataset = VOCDetection(args.dataset_root, [('2007', set_type)],
    ...
    with open('mean_new_ap.txt', 'a+') as f:
        f.write(str(args.trained_model) + ": " + str(mean_ap))
        f.write("\n")
